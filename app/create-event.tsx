...
export default function CreateEventScreen() {
  ...
  return <Screen><BackButton/><View style={styles.heading}><Text style={styles.title}>Create an event</Text><Text style={styles.sub}>Just a name. That's it.</Text></View><GlassCard style={styles.formCard}><GlassInput label="Event name" value={name} onChangeText={setName} placeholder="Ladakh Trip 2025" /><Text style={styles.hint}>You can invite everyone after it's created.</Text></GlassCard>{error?<Text style={styles.error}>{error}</Text>:null}<Pressable disabled={loading} onPress={create} focusable={false} android_ripple={{color:'transparent'}} style={({pressed})=>[styles.createButton,pressed&&styles.pressed,loading&&styles.disabled]}><View style={styles.buttonContent}><Text style={styles.createButtonLabel}>{loading?'Creating…':'Create event'}</Text><MaterialCommunityIcons name="arrow-right" size={24} color={colors.black}/></View></Pressable></Screen>;
}
const styles=StyleSheet.create({heading:{marginTop:34,paddingBottom:3},title:{color:colors.white,fontSize:34,fontWeight:'800',letterSpacing:-1},sub:{color:colors.muted,fontSize:15,marginTop:7},formCard:{padding:14},hint:{color:colors.faint,fontSize:12,marginTop:12,marginHorizontal:2},error:{color:colors.danger,fontSize:13,marginHorizontal:4},createButton:{height:66,borderRadius:radii.pill,backgroundColor:'rgba(250,252,255,0.98)',borderWidth:1,borderColor:'rgba(255,255,255,0.86)',alignItems:'center',justifyContent:'center',...shadows},buttonContent:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10},createButtonLabel:{color:colors.black,fontSize:16,fontWeight:'700',letterSpacing:-0.15},pressed:{transform:[{scale:0.985}],opacity:0.88},disabled:{opacity:0.55}});
