...
export default function EventScreen() {
  ...
  useEffect(() => {
    const target = tab === 'photos' ? photoScrollOffset.current : 0;
  ...
  return (
    <View style={styles.root}>
      <View pointerEvents="none" style={styles.background}>
        ...
        <LinearGradient colors={['rgba(4,9,14,.02)', 'rgba(4,9,14,.08)', 'rgba(8,16,23,.72)']} locations={[0, .48, 1]} style={StyleSheet.absoluteFillObject} />
      </View>
      ...
      >
        ...
        <View style={styles.gallery}>
          <BlurView pointerEvents="none" intensity={24} tint="dark" style={styles.galleryBlur} />
          <LinearGradient pointerEvents="none" colors={['rgba(8,16,23,0)', 'rgba(8,16,23,.08)', 'rgba(8,16,23,.20)']} locations={[0, .25, 1]} style={styles.galleryFade} />
          {error ? <Text style={styles.error}>{error}</Text> : null}
          {tab === 'photos' ? (
            <View style={styles.grid}>
              {galleryPhotos.map((photo, index) => <Pressable key={photo.id || index} disabled={photo.placeholder} style={styles.photo} onPress={() => router.push({ pathname: '/photo/[id]', params: { id: photo.id, eventId: id, index: String(index) } })}>
...
const styles = StyleSheet.create({
  ...
  title: { color: colors.white, fontSize: 29, lineHeight: 35, fontWeight: '800', letterSpacing: -.7 },
  ...
  tabsSticky: { height: 86, paddingHorizontal: 20, paddingTop: 9, paddingBottom: 9, backgroundColor: 'transparent', zIndex: 10 },
  tabs: { width: '100%', height: 68, borderRadius: 34, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,.38)', backgroundColor: 'rgba(170,185,196,.18)', ...shadows },
  tabsGlass: { flex: 1, padding: 7, borderRadius: 34, backgroundColor: 'rgba(190,205,215,.20)', borderWidth: 1, borderColor: 'rgba(255,255,255,.25)', flexDirection: 'row' },
  tab: { flex: 1, borderRadius: 30, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 14 },
  activeTab: { backgroundColor: 'rgba(255,255,255,.98)', borderWidth: 1, borderColor: 'rgba(255,255,255,.98)', shadowColor: '#fff', shadowOpacity: .5, shadowRadius: 10, shadowOffset: { width: 0, height: 1 }, elevation: 5 },
  tabText: { color: 'rgba(255,255,255,.94)', fontSize: 18, fontWeight: '700' },
  activeTabText: { color: colors.black, fontSize: 18, fontWeight: '800' },
  gallery: { position: 'relative', paddingHorizontal: 10, paddingTop: 0, backgroundColor: 'transparent' },
  galleryBlur: { position: 'absolute', top: -110, bottom: 0, left: 0, right: 0 },
  galleryFade: { position: 'absolute', top: -110, bottom: 0, left: 0, right: 0 },
