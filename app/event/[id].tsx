...
export default function EventScreen(){
  ...
  const invite=async()=>{const link=`https://mefie.app/e/${event?.invite_code||''}`;await Share.share({message:`Join ${event?.name||'our Mefie event'} 📸\nEveryone's photos go into one shared album.\n\n${link}`});};
  const title=event?.name||'Event';
  return <View style={styles.root}><Screen backgroundImage={photos[0]?.public_url||fallbackPhoto}>
    <View style={styles.top}><BackButton/><IconButton accessibilityLabel="Invite friends" onPress={invite}><MaterialCommunityIcons name="dots-horizontal" size={22} color={colors.white}/></IconButton></View>
    ...
    <GlassButton label="Invite friends" icon={<MaterialCommunityIcons name="link-variant" size={18} color={colors.white}/>} onPress={invite}/>
    {error?<Text style={styles.error}>{error}</Text>:null}
    <GlassCard style={styles.galleryCard}><View style={styles.tabs}><Pressable onPress={()=>setTab('photos')} style={[styles.tab,tab==='photos'&&styles.activeTab]}><MaterialCommunityIcons name="image-multiple-outline" size={17} color={tab==='photos'?colors.black:colors.muted}/><Text style={tab==='photos'?styles.activeTabText:styles.tabText}>Photos</Text></Pressable><Pressable onPress={()=>setTab('people')} style={[styles.tab,tab==='people'&&styles.activeTab]}><MaterialCommunityIcons name="account-group-outline" size={17} color={tab==='people'?colors.black:colors.muted}/><Text style={tab==='people'?styles.activeTabText:styles.tabText}>People</Text></Pressable></View>
  ...
  </Screen><Pressable accessibilityRole="button" accessibilityLabel="Take a photo" onPress={()=>router.push({pathname:'/camera/[eventId]',params:{eventId:id}})} style={styles.camera}><MaterialCommunityIcons name="camera-outline" size={28} color={colors.black}/></Pressable></View>;
}
