...
export default function CameraScreen(){
  ...
  const capture=async()=>{
    ...
    try{const photo=await ref.current.takePictureAsync({quality:0.9,skipProcessing:false});if(!photo?.uri)throw new Error('Could not capture the photo.');await upload(photo.uri,photo.width,photo.height);await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);setMessage('Shared ✓');}catch(e:any){setMessage(e?.message||'Upload failed.')}finally{setBusy(false)}
  };
