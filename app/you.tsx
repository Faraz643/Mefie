...
export default function You() {
  ...
  return <View style={styles.root}>
    <Screen>
      ...
      <GlassCard style={styles.backgroundCard}>
        <View style={styles.backgroundHeader}>
          ...
          <View style={styles.backgroundCopy}>
            ...
            <Text style={styles.backgroundSub}>Choose a photo from your phone</Text>
          </View>
        </View>
        {backgroundImage ? <Image source={{ uri: backgroundImage }} style={styles.preview} resizeMode="cover" /> : null}
        <GlassButton primary label={backgroundImage ? 'Change background' : 'Choose background photo'} onPress={chooseBackground} />
      ...
      {editing ? <GlassCard><GlassInput label="Display name" value={name} onChangeText={setName} /><View style={{ marginTop: 14 }}><GlassButton primary label="Save" onPress={async () => { await setDisplayName(name); setEditing(false); }} /></View></GlassCard> : <GlassButton label="Edit profile" onPress={() => setEditing(true)} />}
    </Screen>
...
function MenuRow({ icon, label, onPress }: { icon: any; label: string; onPress?: () => void }) {
  return <Pressable style={styles.menuRow} onPress={onPress} disabled={!onPress}>
    <MaterialCommunityIcons name={icon} size={21} color="rgba(255,255,255,.82)" />
    <Text style={styles.rowText}>{label}</Text>
    <MaterialCommunityIcons name="chevron-right" size={21} color="rgba(255,255,255,.55)" />
  </Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  title: { color: '#fff', fontSize: 29, fontWeight: '800', marginTop: 14, letterSpacing: -.5 },
  handle: { color: colors.muted, marginTop: 3, fontSize: 13 },
  menu: { paddingHorizontal: 16, paddingVertical: 8 },
