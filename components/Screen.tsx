...
export function Screen({
  ...
  const source = typeof backgroundImage === 'string' ? { uri: backgroundImage } : backgroundImage;
...
export function BottomNav({ active = 'home' }: { active?: 'home' | 'events' | 'you' }) {
  ...
  return (
    <View style={[styles.nav, { bottom: Math.max(insets.bottom + 10, 18) }]}>
      {items.map(({ key, label, icon, path }) => (
        <Pressable key={key} onPress={() => router.replace(path)} style={styles.navItem}>
          <View style={[styles.navIconWrap, active === key && styles.navIconActive]}>
            <MaterialCommunityIcons
              name={icon}
              size={24}
              color={active === key ? colors.white : 'rgba(255,255,255,0.55)'}
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  nav: {
    ...
    minHeight: 76,
    borderRadius: 27,
    backgroundColor: 'rgba(22,29,38,0.72)',
    flexDirection: 'row',
